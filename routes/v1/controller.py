...
from domain.use_cases.factory_usecases.factory_save_application_of_end_weighting_use_case import \
    FactorySaveApplicationOfEndWeightingUseCase
...
class MainControllerV1:
    ...
    async def save_application(self, save_application_request: SaveApplicationRequest) -> SaveApplicationResponse:
    ...
    async def get_scales_names(self):
        scalesType: list[ScalesParam] = await self.__get_scales_type_usecase()
    ...
    async def get_orders_list(self):
        orders: list[Order] = await self.__get_orders_usecase()
    ...
    async def get_equipments_list(self):
        equipment: list[Equipment] = await self.__get_equipment_usecase()
